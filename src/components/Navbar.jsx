        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
              ...
              {navLinks.map((link) => (
                <Link key={link.name} to={link.path}
                  className={`flex items-center px-3 py-2 text-sm font-medium rounded-lg transition-all duration-200 ${
                    isActive(link.path) ? 'text-prepzo-green bg-prepzo-light-green' : 'text-gray-600 hover:text-prepzo-green hover:bg-prepzo-light-green/50'
                  }`}>
                  {link.name}
                  {link.hasDropdown && <ChevronDown className="ml-1 h-3.5 w-3.5 opacity-60" />}
                </Link>
              ))}
            </div>
            ...
            <button id="mobile-menu-btn" onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
              className="md:hidden p-2 rounded-lg text-gray-600 hover:text-prepzo-green hover:bg-prepzo-light-green transition-colors">
              {isMobileMenuOpen ? <X className="h-5 w-5" /> : <Menu className="h-5 w-5" />}
            </button>
          </div>
        </div>

        <div className={`md:hidden transition-all duration-300 overflow-hidden border-t border-gray-100 bg-white ${isMobileMenuOpen ? 'max-h-screen opacity-100' : 'max-h-0 opacity-0'}`}>
